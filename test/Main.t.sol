...
contract MainTest is Test {
    ...
    function testProvideLiquidity() public {
        ...
        (uint256 tokenId,,) = main.provideLiquidity(WETH_USDC_POOL, usdcAmount, wethAmount, width, 60);
    ...
    function testWidthTooLarge() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Main.WidthTooLarge.selector, width));

        main.provideLiquidity(WETH_USDC_POOL, 1000 * 10 ** 6, 1 ether, width, 50);
    ...
    function testCalculateLiquidity() public view {
        ...
        uint128 liquidity = main.calculateLiquidity(1000 * 10 ** 6, 1 ether, sqrtPriceX96, lowerTick, upperTick);
    ...
    function testCollectFees() public {
        ...
        (uint256 tokenId,,) = main.provideLiquidity(WETH_USDC_POOL, 2000 * 10 ** 6, 1 ether, 500, 60);
        ...
        assertGt(IERC20(USDC).balanceOf(user), initialUSDC, "USDC fees not collected");
        assertGt(IERC20(WETH).balanceOf(user), initialWETH, "WETH fees not collected");
    ...
    function testIncreaseLiquidity() public {
        ...
        (uint256 addedLiquidity,,) = main.increaseLiquidity(tokenId, 1000 * 10 ** 6, 0.5 ether, 60);

        assertGt(addedLiquidity, 0, "Liquidity not increased");
        assertGt(_getPositionLiquidity(tokenId), initialLiquidity, "Total liquidity mismatch");
    ...
    function testWithdrawFullLiquidity() public {
        ...
        assertApproxEqAbs(IERC20(USDC).balanceOf(user), balanceBeforeUSDC + amount0, 1000, "USDC balance mismatch");
        assertApproxEqAbs(
            IERC20(WETH).balanceOf(user), balanceBeforeWETH + amount1, 0.01 ether, "WETH balance mismatch"
        );
    ...
    function testAmountTooSmall() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Main.AmountTooSmall.selector, 0));
    ...
    function testTokenReturn() public {
        ...
        (, uint256 usdcUsed, uint256 wethUsed) = main.provideLiquidity(WETH_USDC_POOL, 4000 * 10 ** 6, 2 ether, 500, 60);

        uint256 finalUsdcBalance = IERC20(USDC).balanceOf(user);
        uint256 finalWethBalance = IERC20(WETH).balanceOf(user);
        assertEq(initialUsdcBalance - usdcUsed, finalUsdcBalance, "USDC not returned correctly");
        assertEq(initialWethBalance - wethUsed, finalWethBalance, "WETH not returned correctly");
    ...
    function testNotOwner() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Main.NotTheOwner.selector, attacker, user));
    ...
    function _swap(IUniswapV3Pool pool, address tokenIn, address tokenOut, uint256 amountIn) internal {
        bool zeroForOne = tokenIn < tokenOut;
        int256 amountSpecified = int256(amountIn);

        (uint160 sqrtPriceX96,,,,,,) = pool.slot0();
        uint160 sqrtPriceLimitX96 =
            zeroForOne ? sqrtPriceX96 - (sqrtPriceX96 / 100) : sqrtPriceX96 + (sqrtPriceX96 / 100);

        pool.swap(address(this), zeroForOne, amountSpecified, sqrtPriceLimitX96, abi.encode(tokenIn, tokenOut));
    }
